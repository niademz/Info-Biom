...
import { useUser } from "@clerk/nextjs"; // Import user context
...
const SpotifyTrackCard: React.FC<SpotifyTrack> = ({ id, name, artists, album, external_urls }) => {
    const { user } = useUser();
  ...
  const [shareVisible, setShareVisible] = useState(false); // State for share menu
    ...
    const handleToggleFavorite = async () => {
        ...
        const contentType = 'spotify_track'; // Set content type to 'spotify_track'
  ...
  const fetchFriends = async () => {
    ...
    return (
        <div style={styles.cardContainer}>
            <iframe 
                src={`https://open.spotify.com/embed/track/${id}`} // Embed the track
                width="100%" 
...
const styles: { [key: string]: React.CSSProperties } = {
    ...
    iframe: {
        ...
        marginBottom: '-50px', // Ensure no margin at the bottom
        padding: '0px', // Remove any potential padding
    },
    trackInfo: {
        padding: '0px', // Add padding only if you want some spacing
        marginTop: '0px', // Ensure no top margin to remove space
        color: 'white',
        textAlign: 'center',
    },
    trackName: {
